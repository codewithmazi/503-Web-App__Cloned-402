...
import { Operation } from '../../src/types/Operation'; // Import the TypeScript type for Movie objects.
...
export async function getOps() {
  try {
    const response = await fetch(`${API_BASE_URL}/operational`, { method: "GET" }); // Sends GET requests to the movies endpoint.
 ...
 * @param {string} log_id - The ID of the movie to fetch.
 * @returns {Promise<any>} - The movie data or null in case of an error.
 */
export async function getCropById(log_id: string) {
  try {
    const response = await fetch(`${API_BASE_URL}/operational/${log_id}`, { method: "GET" }); // Sends GET request to the specific movie endpoint.
...
const getAll = async () => {
  try {
    ...
    return http.get<Array<Operation>>("/operational", {
      headers: {
 ...
 */
const get = async (log_id: string) => {
  try {
    // Fetches authentication tokens from AWS Amplify.
    const { idToken } = (await fetchAuthSession()).tokens ?? {};
    return http.get<Operation>(`/operational/${log_id}`, {
      headers: {
        Authorization: idToken?.toString(), // Attaches the ID token as the Authorization header.
      },
    });
...
const opsService = {
 // getCropById, // Publicly expose `getMovieById` function.
  getOps,    // Publicly expose `getMovies` function.
